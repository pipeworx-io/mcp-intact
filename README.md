...
`tools/list` at `https://gateway.pipeworx.io/intact/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/find_interactions \
  -H 'Content-Type: application/json' \
  -d '{"query":"EGFR_HUMAN"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/find_interactions`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
